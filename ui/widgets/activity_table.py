...
class ActivityTable(QTableWidget):
    ...
    delete_limit_requested = pyqtSignal(str)  # app_name
    ...
    def _on_double_click(self, row: int, column: int):
        """При двойном клике на приложении без лимита — предложить добавить."""
        app_name = self.item(row, 1).text() if self.item(row, 1) else ''
        if app_name:
            self.add_limit_requested.emit(app_name)
    ...
    def _on_context_menu(self, pos):
        ...
        app_name_item = self.item(row, 1)
        if not app_name_item:
            return
        ...
        action_delete.triggered.connect(lambda: self.delete_limit_requested.emit(app_name))
