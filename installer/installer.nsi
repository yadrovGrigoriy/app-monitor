...
OutFile "..\dist\v1.2.25\AppMonitor_Setup_${PRODUCT_VERSION}.exe"
...
Section "!ќсновные файлы" SecCore
    SectionIn RO
    SetOutPath "$INSTDIR"
    File "..\dist\v1.2.25\AppMonitor.exe"
...
Section "ярлык в меню ѕуск" SecStartMenu
    CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
    CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\AppMonitor.lnk" "$INSTDIR\AppMonitor.exe" "" "$INSTDIR\AppMonitor.exe" 0
    CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\”далить AppMonitor.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
SectionEnd
...
Section "јвтозапуск (рекомендуетс€)" SecAutostart
...
LangString DESC_SecCore ${LANG_RUSSIAN} "ќсновные файлы приложени€ (об€зательно)"
LangString DESC_SecCore ${LANG_ENGLISH} "Core program files (required)"
LangString DESC_SecStartMenu ${LANG_RUSSIAN} "—оздать €рлык в меню ѕуск"
LangString DESC_SecStartMenu ${LANG_ENGLISH} "Add shortcuts to Start Menu"
LangString DESC_SecDesktop ${LANG_RUSSIAN} "—оздать €рлык на рабочем столе"
LangString DESC_SecDesktop ${LANG_ENGLISH} "Add shortcut to Desktop"
LangString DESC_SecAutostart ${LANG_RUSSIAN} "јвтоматически запускать AppMonitor при входе в Windows"
...
Section "Uninstall"
    ...
    IfFileExists "$APPDATA\AppMonitor\*.*" 0 +3
        MessageBox MB_YESNO|MB_ICONQUESTION "”далить данные в AppData ($APPDATA\AppMonitor)?$\n$\n¬нимание: будут удалены все логи и Ѕƒ!" IDNO +2
