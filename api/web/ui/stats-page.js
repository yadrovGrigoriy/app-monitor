/**
 * Компонент: статистика за период.
 */
const StatsPage = {
    template: `
        <div>
            ...
            <a-row :gutter="[12, 12]" style="margin-bottom:20px">
                <a-col :xs="12" :sm="12">
                    <a-card size="small" class="stat-card">
                        <statistic :value="formatDuration(stats.total_seconds)" title="Всего за период" />
                    </a-card>
                </a-col>
                <a-col :xs="12" :sm="12">
                    <a-card size="small" class="stat-card">
                        <statistic :value="stats.apps.length" title="Приложений" />
                    </a-card>
                </a-col>
            </a-row>
                ...
                <a-table :dataSource="stats.apps" :columns="columns" :pagination="false"
                    ...
                    <template #bodyCell="{ column, record, index }">
                        ...
                        <template v-if="column.key === 'bar'">
                            ...
                            <span class="progress-label">{{ percentOfTotal(record.duration_seconds, stats.total_seconds) }}%</span>
                        </template>
                    </template>
                </a-table>
            </a-card>
    ...
    setup(props, { emit }) {
        ...
        return { startDate, endDate, columns, loadStats, ...AppMonitorUtils };
    },
};
